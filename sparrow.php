...
elseif ($ip == "fix")
  {
    ...
    echo $bold . $fgreen . "[i] Job finished successfully! Please Restart Sparrow \n";
    exit;
  }
    ...
    if (!in_array($scan, array(
    ...
    else
      {
        ...
        elseif ($scan == 'q' | $scan == 'Q')
          {
            echo "\n\n\t GOD BLESS YOU MY HOLY CHILD - Have a nice day <3\n\n";
            die();
          }
        ...
        elseif ($scan == "0")
          {
            ...
            echo "\n\n";
            echo $bold . $yellow . "[*] Scanning Complete. Press Enter\n\n";
                ...
                if ($wp_version != "")
                  {
                    ...
                    if (strpos($vuln_json, "The page you were looking for doesn't exist (404)") !== false)
                      {
                        echo $red . "[!] Seems like the version detection messed up preety bad!";
                      }
                    else
                      {
                  ...
                  {
                    $reallink = $ipsl . $ip;
                    echo $red . "Failed \n\n[!] We could not determine the WordPress version of the target!";
                    echo "\n\n";
                    echo $bold . $yellow . "[*] Scanning Complete. Press Enter\n\n";
                    trim(fgets(STDIN, 1024));
                    goto scanlist;
        ...
        elseif ($scan == 'U' || $scan == 'u')
          {
            echo "\n\n" . $bold . $yellow . "-[Sparrow Update Corner]-\n\n" . $cln;
            echo $bold . "[i] Fetching Stuffs .... \n" . $cln;
            $latestversion = readcontents("https://raw.githubusercontent.com/RIDS_SECURITY/Sparrow/version.txt");
            echo $bold . $blue . "[C] Current Version: " . $rhversion . $cln;
            echo "\n" . $bold . $lblue . "[L] Latest Version:  " . $latestversion . $cln;
            if ($latestversion > $rhversion)
              {
                echo $bold . $fgreen . "\n\n[U] Update Available, Please Update Your Version Of Sparrow \n" . $cln;
                echo $bold . $white . "    Link: link to file rk.php\n\n" . $cln;
              }

            elseif ($rhversion == $latestversion)
              {
                echo $bold . $fgreen . "\n[i] You are already running the latest version of Sparrow. \n\n" . $cln;
              }
            else
              {
                echo $bold . $red . "\n[U]WHY THE HECK U TAMPERED MY SCRIPT !! You are being watched from now onwards \n\n";
              }
          }
        elseif ($scan == "F" || $scan == "f"){
          ...
          echo $bold . $fgreen . "[i] READY TO GO !! Please Restart Sparrow \n";
          exit;
        }
